...
export default function handler(options: any): Rule {
  const templateSource = apply(url('./files/customHeading'), [
    applyTemplates({
      ...strings,
      ...options
    }),
  ...
  return chain([
    ...
    App.make(WordpressService).declareInServiceProvider('app/Providers/BlockServiceProvider.php', `\\App\\Blocks\\CustomHeadingBlock::class,`)
  ]);
}
